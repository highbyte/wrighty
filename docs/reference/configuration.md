...
| `worker.useWorkerQueue` | `true` | The pick-from status (`defaultPickFrom`, `"Agent queue"` by default) is the worker queue: placing an item there is the automatic-execution authorization. An operator moving an item into the queue through a Wrighty surface (CLI `move`/`edit`, the web dashboard) sets the execution policy to automatic, moving it out revokes it, and a running worker's poll authorizes any item it finds in that status — which covers a GitHub board drag that no Wrighty surface observed (entry-only there; drag out of the column is equally unobserved, so revocation on the board remains an explicit policy edit). The worker's own status moves — pick, finish, refusal-restore — never trigger the rule, and an explicitly patched execution policy always wins. **Keep the pick-from a dedicated queue status**: pointing it at a general-purpose `Todo` authorizes everything already sitting there on the next poll. On GitHub, `wrighty init` adds the missing pick-from option to the Status field for you, inserted directly after the first option so the board reads Todo → queue → In Progress; existing options are never reordered or removed. GitHub context approval is never affected. Set `false` to keep the execution policy a separate explicit edit (and pick your own `defaultPickFrom`). |
