...
| `worker.requirementsAssessment.mode` | `enforced` | Requirements-readiness check for fresh worker sessions. `enforced` runs a restricted first turn and resumes the same session only after a valid ready verdict; `inline` keeps the lower-cost single-turn behavioral guard; `off` is a compatibility escape hatch and emits a startup warning. See [Requirements-readiness assessment](worker.md#requirements-readiness-assessment). |
