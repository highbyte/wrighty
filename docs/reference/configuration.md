...
| `worker.sessionReportMode` | `off` | Whether a finished run publishes a report comment on the item: `off` (default), `completed` (only runs Wrighty observed reaching the completion status), or `all` (also needs-attention, failed and rejected runs). Publishing writes to a surface collaborators read, so it stays off until asked for. A usage-deferred run publishes nothing; its scheduled retry reports for itself. |
