...
| `enabledAgents` | Web console **Agents** menu | (unset → every detected agent) | Explicit allowlist for agents eligible for automatic Wrighty-managed work on this computer. Effective enablement always also requires the CLI to be detected. The first toggle materializes the complete detected set before applying the change; later installations are therefore not silently enabled. If a selected CLI temporarily disappears, Wrighty retains the preference but treats the agent as disabled until it is detected again. An explicit `--agent` selection remains a one-run override for a detected agent. |
