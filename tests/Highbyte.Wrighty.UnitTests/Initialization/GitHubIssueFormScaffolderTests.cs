...
public sealed class GitHubIssueFormScaffolderTests
{
    ...
    public async Task Scaffold_creates_only_neutral_form_with_chooser_config()
    {
        ...
        try
        {
            ...
            Assert.Contains("Wrighty policy - agent", task);
            Assert.Contains("Wrighty policy - execution", task);
    ...
    public async Task Scaffold_is_idempotent_and_does_not_overwrite_conflicts()
    {
        ...
        try
        {
            ...
            Assert.Equal("blank_issues_enabled: true\n", await File.ReadAllTextAsync(
                Path.Combine(directory, "config.yml")));
            Assert.Contains(result.Actions, action => action.Contains(
                "Did not overwrite conflicting Wrighty issue-template chooser"));
