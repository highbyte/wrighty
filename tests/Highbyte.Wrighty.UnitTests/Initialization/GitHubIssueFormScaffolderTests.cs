...
{
    [Fact]
    public async Task Scaffold_creates_only_neutral_form_with_chooser_config()
    {
        ...
        try
        {
            ...
            Assert.False(File.Exists(Path.Combine(directory, "wrighty-claude.yml")));
            Assert.False(File.Exists(Path.Combine(directory, "wrighty-codex.yml")));
            Assert.False(File.Exists(Path.Combine(directory, "wrighty-copilot.yml")));
            Assert.False(File.Exists(Path.Combine(directory, "wrighty-default-agent.yml")));
            ...
            Assert.Contains("Preferred agent", task);
            Assert.Contains("Worker execution", task);
            Assert.Contains("blank_issues_enabled: false", chooser);
            Assert.Contains(result.Actions, action => action.StartsWith("created Wrighty backlog"));
    ...
    public async Task Scaffold_is_idempotent_and_does_not_overwrite_conflicts()
    {
        ...
        try
        {
            ...
            Assert.Equal("blank_issues_enabled: true\n", await File.ReadAllTextAsync(
                Path.Combine(directory, "config.yml")));
            Assert.Contains(result.Actions, action => action.Contains("Did not remove customized legacy"));
    ...
    public async Task Scaffold_refreshes_an_unchanged_generated_form_for_a_new_project()
    {
        ...
        try
        {
            ...
            var task = await File.ReadAllTextAsync(Path.Combine(directory, "wrighty-task.yml"));
            Assert.Contains("owner/12", task);
            Assert.DoesNotContain("owner/11", task);
            Assert.Contains(result.Actions, action => action.StartsWith("updated Wrighty backlog"));
        }
        finally
        {
            Directory.Delete(root, recursive: true);
    ...
    [Fact]
    public async Task Scaffold_retires_exact_generated_legacy_form()
    {
        var root = Path.Combine(Path.GetTempPath(), $"wrighty-forms-{Guid.NewGuid():N}");
        var directory = Path.Combine(root, ".github", "ISSUE_TEMPLATE");
        Directory.CreateDirectory(directory);
        var legacyPath = Path.Combine(directory, "wrighty-default-agent.yml");
        await File.WriteAllTextAsync(
            legacyPath,
            """
            # Generated by Wrighty. Rerun 'wrighty init' to refresh the Project reference.
            name: Wrighty worker task (default agent)
            description: Create work explicitly authorized for processing by the worker's default agent
            labels:
              - "wrighty:auto"
            projects:
              - "owner/12"
        ...
        try
        {
            ...
            Assert.Contains(legacyPath, result.ManagedPaths);
            Assert.Contains(legacyPath, result.ChangedPaths);
            Assert.Contains(result.Actions, action => action.StartsWith("removed legacy"));
        }
        finally
        {
            Directory.Delete(root, recursive: true);
