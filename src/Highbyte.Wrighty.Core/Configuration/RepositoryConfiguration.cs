...
internal static class ConfigurationCatalogue
{
        ...
        TrackerConfig config)
    {
        var values = new List<ConfigurationSettingDescriptor>
        {
            ...
            Setting(root, "worker.requirementsAssessment.mode",
            ...
            Setting(root, "worker.continuation.trigger",
                ...
                ConfigurationEditMode.Ordinary, ConfigurationEffectiveBoundary.NewWorker,
                "What a trusted author's reply must be to continue a waiting session."),
            Setting(root, "worker.continuation.command",
                config.EffectiveWorker.EffectiveContinuation.Command, "string",
                ConfigurationEditMode.Ordinary, ConfigurationEffectiveBoundary.NewWorker,
                "Exact first-line control command required by command-only mode."),
            Setting(root, "worker.continuation.resumeReaction",
                config.EffectiveWorker.EffectiveContinuation.ResumeReaction, "string",
                ConfigurationEditMode.Ordinary, ConfigurationEffectiveBoundary.NewWorker,
                "GitHub reaction that resumes from the current needs-attention status comment."),
            Setting(root, "worker.continuation.completionReaction",
                config.EffectiveWorker.EffectiveContinuation.CompletionReaction, "string",
                ConfigurationEditMode.Ordinary, ConfigurationEffectiveBoundary.NewWorker,
                "GitHub reaction that asks the retained agent to verify and finish."),
            Setting(root, "worker.continuation.maxAutomaticContinuations",
                config.EffectiveWorker.EffectiveContinuation.MaxAutomaticContinuations, "integer",
                ConfigurationEditMode.Ordinary, ConfigurationEffectiveBoundary.NewWorker,
